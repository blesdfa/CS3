...
void Autocompleter::insert(string x, int freq) {
    ...
    root->top.push_back(e1); // After this assignment it's very apparent I need to follow DRY coding(dont repeat yourself) 
    int j = root->top.size() - 1; // logic for keeping track of all top entries 
    while(j > 0 && root->top[j].freq > root->top[j - 1].freq) {
        swap(root->top[j], root->top[j - 1]);
        j--;
    }
    ...
    for (int i = 0; i < x.size(); i++)
    {
        ...
        temp->top.push_back(e1);
        j = temp->top.size() - 1;
        while(j > 0 && temp->top[j].freq > temp->top[j - 1].freq) {
            swap(temp->top[j], temp->top[j - 1]);
            j--;
        }
...
void Autocompleter::completions(string x, vector<string> &T) {
    T.clear();
    Node* temp = root; // first I was going to make an edge case to check if the string was empty but realized the for loop wouldnt iterate because the size would be 0. So it goes straight to just populating at whatever node "temp" is set to which would be root from the beginning 
